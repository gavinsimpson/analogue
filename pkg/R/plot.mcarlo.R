## plot.mcarlo <- function(x, which = c(1:2),
...
##     on.exit(par(op))
##   }
##   xlable <- paste("Dissimilarity (", attr(x, "method"), ")", sep = "")
...
##     hist(x, freq = FALSE, xlim = c(0, range.dists),
##          main = "", xlab = "", ylab = "", ann = FALSE)
##     box()
##     title(main = caption[1],
##           ylab = "Density", xlab = xlable)
##     lines(density(x, adjust = 2, from = 0, to = range.dists))
##   }
##   if(show[2]) {
##     num.dists <- length(x)
##     cumfreq <- sapply(evalDists, function(x, y) length(y[y <= x]), x) / num.dists
...
##           ylab = "Probability", xlab = xlable)
##     polygon(c(0, sigx[,1], sigx[nrow(sigx),1]), c(0, sigx[,2], 0),
